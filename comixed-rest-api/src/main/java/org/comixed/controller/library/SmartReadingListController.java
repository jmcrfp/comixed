...
@RestController
@RequestMapping("/api")
public class SmartReadingListController {
  ...
  @RequestMapping(value = "/smartlists", method = RequestMethod.POST)
  ...
  public SmartReadingList createSmartReadingList(
    ...
    this.logger.info("Creating smart reading list for user: email={} name={}", email, name);

    // TODO joao
    return this.readingListService.createSmartReadingList(email, name, summary, false, null, null);
  }

//  @PutMapping(value = "/smartlists/{id}", produces = "application/json", consumes = "application/json")
//  @JsonView(View.SmartReadingList.class)
//  public SmartReadingList updateSmartReadingList(
//      Principal principal,
...
//
//    this.logger.info(
//        "Updating smart reading list for user: email={} id={} name={} summary={}",
//        email,
//        id,
//        name,
//        summary);
...
//  }
  ...
  @RequestMapping(value = "/smartlists", method = RequestMethod.GET)
  @JsonView(View.SmartReadingList.class)
  public List<SmartReadingList> getSmartReadingListsForUser(Principal principal) {
    if (principal == null) {
      return null;
    }
    final String email = principal.getName();
    ...
    final List<SmartReadingList> result = this.readingListService.getSmartReadingListsForUser(email);

    this.logger.debug("Returning {} smart lists{}", result.size(), result.size() == 1 ? "" : "s");
    return result;
  }

  @GetMapping(value = "/smartlists/{id}", produces = "application/json")
  @JsonView(View.SmartReadingList.class)
  public SmartReadingList getSmartReadingList(final Principal principal, @PathVariable("id") final long id)
      throws NoSuchReadingListException {
    final String email = principal.getName();
    this.logger.info("Getting reading list for user: email={} id={}", email, id);

    return this.readingListService.getSmartReadingListForUser(email, id);
  }
}
